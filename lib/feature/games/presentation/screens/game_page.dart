...
import 'package:vozhaomuz/shared/widgets/game_energy_hud.dart';
...
class _GamePageState extends ConsumerState<GamePage> {
  ...
  /// ResultGamePages. Symptom: in course flow Exit needs two taps because
  /// the first only pops the duplicate. Reset back to false after the
  /// pushed route returns so a subsequent session can navigate again.
  ...
  Future<void> _finishGame(BuildContext context, WidgetRef ref) async {
    // Guarded both here AND at the Builder call-site (see build()) so a
    // burst of rebuilds with stage=result can't queue two microtasks
    // racing to push.
    ...
    // Don't await + reset _resultPushed: when popUntil from
    // ResultGamePage's Exit pops GamePage too, the await unblocks AFTER
    // sync code finishes, possibly while another microtask sits in the
    // queue. Resetting the flag would let that microtask push a fresh
    // ResultGamePage. The flag stays true for the rest of this State's
    // lifetime; a new session creates a new State with a fresh flag.
  ...
  Widget build(BuildContext context) {
    ...
    return userAsync.when(
      ...
      data: (user) {
        ...
        return PopScope(
          canPop: ref.watch(allowGameFlowPopProvider),
          onPopInvokedWithResult: (didPop, result) {
            if (!didPop) {
              showExitConfirmationDialog(context, onExit: () {
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            showExitConfirmationDialog(context, onExit: () {
                              ref.read(isRepeatModeProvider.notifier).set(false);
                              ref.read(allowGameFlowPopProvider.notifier).set(
                                true,
                              );
                            });
                          },
...
dynamic getNextStage(GameStage current, WidgetRef ref) {
  ...
  if (isRepeat) {
    ...
    // Агар калимаҳо холӣ бошанд — ин бозиро гузаронида, бозии навбатиро интихоб мекунем
    if (nextWords.isEmpty) {
      debugPrint('⚠️ [getNextStage] Repeat game "$nextGameName" has 0 words, skipping');
      if (nextIdx + 1 >= gameOrder.length) {
        return GameStage.result;
      }
      // Рекурсия бо индекси навбатӣ
      ref.read(repeatGameIndexProvider.notifier).set(nextIdx);
      return getNextStage(nextStage, ref);
    }
